...
RSpec.describe Inquiry::SearchClause do
  ...
  describe "initialization" do
    it "takes the search key, filter clause, and optional options" do
      ...
      expect(described_class.new(:first_name, "first_name LIKE ?", fuzzy: true)).to be_a described_class
    end
  end
  ...
  describe "#apply" do
    ...
    it "does a fuzzy match when configured to do so" do
      subject = described_class.new(:first_name, "first_name LIKE ?", fuzzy: true)
    ...
    it "applies the search match value as many times as it is referenced" do
      subject = described_class.new(:name, "first_name LIKE ? OR last_name LIKE ?", fuzzy: true)
